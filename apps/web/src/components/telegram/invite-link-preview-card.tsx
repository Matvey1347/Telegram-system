...
export function InviteLinkPreviewCard({
  ...
  const shouldLoadHistory =
    showHistoryPreview &&
    !history &&
    Boolean(link.telegramChannelId && link.id);
  const historyQuery = useQuery({
    queryKey: ["invite-link-history", link.telegramChannelId, link.id],
    queryFn: () =>
      getTelegramChannelInviteLinkHistory(link.telegramChannelId, link.id),
    enabled: shouldLoadHistory,
    staleTime: 60_000,
  });
  const resolvedHistory = history ?? historyQuery.data ?? null;
  ...
  return (
    ...
    >
      ...
      {showHistoryPreview && historyQuery.isLoading && !history ? (
        <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950/30 px-3 py-2 text-xs text-slate-500">
          Loading history...
        </div>
      ) : null}

      {showHistoryPreview && !historyQuery.isLoading && !resolvedHistory?.summary ? (
        <div className="mt-3 rounded-lg border border-slate-800 bg-slate-950/30 px-3 py-2 text-xs text-slate-500">
