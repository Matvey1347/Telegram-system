...
import type { MemberSummary, WorkspaceMember } from '@/lib/api';
...
export function MemberBadge({
  ...
  member?: WorkspaceMember | MemberSummary | null;
  compact?: boolean;
}) {
  if (!member) {
  ...
  return (
    <span className="inline-flex min-w-0 items-center gap-2 text-sm text-neutral-200">
      <IconAvatar icon={member.avatarPresentation} label={member.user.name} size="xs" />
