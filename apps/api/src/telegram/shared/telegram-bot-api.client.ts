...
export class TelegramBotApiClient {
  ...
  getChat(token: string, chatIdOrUsername: string) {
    ...
    }>(token, 'getChat', { chat_id: chatIdOrUsername });
  }

  getChatMemberCount(token: string, chatIdOrUsername: string) {
    return this.call<number>(token, 'getChatMemberCount', {
      chat_id: chatIdOrUsername,
    });
  }

  getFile(token: string, fileId: string) {
    return this.call<{ file_path?: string }>(token, 'getFile', {
      file_id: fileId,
    });
  }
  ...
  getChatMember(
    ...
    }>(token, 'getChatMember', { chat_id: chatIdOrUsername, user_id: userId });
  }
  ...
  getChatAdministrators(token: string, chatIdOrUsername: string) {
    ...
    >(token, 'getChatAdministrators', { chat_id: chatIdOrUsername });
  }

  getChatAdministratorsFiltered(
    token: string,
    chatIdOrUsername: string,
    returnBots = true,
  ) {
