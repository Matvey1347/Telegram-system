...
export class TelegramUserAccountsController {
  ...
  @Post(':id/login/start') startLogin(@CurrentUser() user: JwtUser, @Param('id') id: string, @Body() dto: StartLoginDto) { return this.service.startLogin(user.sub, id, dto); }
  @Post(':id/login/code') confirmCode(@CurrentUser() user: JwtUser, @Param('id') id: string, @Body() dto: ConfirmLoginCodeDto) { return this.service.confirmCode(user.sub, id, dto); }
  @Post(':id/login/password') confirmPassword(@CurrentUser() user: JwtUser, @Param('id') id: string, @Body() dto: Confirm2faPasswordDto) { return this.service.confirmPassword(user.sub, id, dto); }
  @Post(':id/check') check(@CurrentUser() user: JwtUser, @Param('id') id: string) { return this.service.check(user.sub, id); }
  @Post(':id/sync-dialogs') syncDialogs(@CurrentUser() user: JwtUser, @Param('id') id: string) { return this.service.syncDialogs(user.sub, id); }
}
