...
export class InvestmentsController {
  ...
  @Post() create(@CurrentUser() user: JwtUser, @Body() dto: CreateInvestmentDto) { return this.service.create(user.sub, dto); }
  @Get(':id') findOne(@CurrentUser() user: JwtUser, @Param('id') id: string) { return this.service.findOne(user.sub, id); }
  @Patch(':id') update(@CurrentUser() user: JwtUser, @Param('id') id: string, @Body() dto: UpdateInvestmentDto) { return this.service.update(user.sub, id, dto); }
