...
Frontend rendering must use `IconAvatar` and pass the hydrated presentation or hydrated legacy `Icon` object from the entity response. Lists, cards, tables, headers and chips must not call `/icons/:id` only to render an existing entity.
...
4. Render with `IconAvatar` or `IconPicker` with hydrated `icon`/presentation passed in.
